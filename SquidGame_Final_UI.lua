...
function Script.Functions.TeleportSafe()
    ...
    Script.Functions.DisableAntiFling()
    Script.Temp.OldLocation = CFrame.new(lplr.Character.HumanoidRootPart.Position)
    lplr.Character:PivotTo(CFrame.new(Vector3.new(80, 50, -150)))
    if call then
        task.delay(0.5, Script.Functions.EnableAntiFling)
    end
end
...
-- ✅ Aimbot Section (Accurate & Locked)
...
local function highlightTarget(player)
    if player and player.Character and player.Character:FindFirstChild("Head") then
        local highlight = player.Character:FindFirstChild("AimbotHighlight") or Instance.new("Highlight")
...
local function removeHighlight()
    for _, p in pairs(Players:GetPlayers()) do
        if p.Character and p.Character:FindFirstChild("AimbotHighlight") then
            p.Character.AimbotHighlight:Destroy()
        end
    end
end

Toggles.EnableAimbot:OnChanged(function(val)
    if val then
        local closest = nil
...
Options.AimbotKey:OnClick(function()
    Toggles.EnableAimbot:SetValue(not Toggles.EnableAimbot.Value)
end)

RunService.RenderStepped:Connect(function()
    if Toggles.EnableAimbot.Value and AimbotTarget and AimbotTarget.Character and AimbotTarget.Character:FindFirstChild("Head") then
...
-- ✅ Teleport to Player Group (Dropdown)
local TeleportGroup = Tabs.Main:AddRightGroupbox("Teleport", "move")
...
TeleportGroup:AddDropdown("TeleportToPlayerList", {
...
Players.PlayerAdded:Connect(function()
    Options.TeleportToPlayerList:SetValues(getPlayerNames())
end)
...
-- ✅ SafeZone Group (UI Toggle)
local SafeGroup = Tabs.Main:AddLeftGroupbox("Safezone", "shield")

SafeGroup:AddToggle("TeleportToSafePlace", {
    Text = "Teleport To Safe Place",
    Default = false
}):AddKeyPicker("TTSPKey", {
    Mode = "Toggle",
    Default = "L",
    Text = "SafeZone Keybind",
    SyncToggleState = true
})

Toggles.TeleportToSafePlace:OnChanged(function(val)
    if val then
